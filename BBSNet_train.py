    ...
    try:
        for i, (images, gts, depths) in enumerate(train_loader, start=1):
            ...
            if i % 100 == 0 or i == total_step or i == 1:
                print(
                    "{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss1: {:.4f} Loss2: {:0.4f}".format(
                        datetime.now(),
                        epoch,
                        opt.epoch,
                        i,
                ...
                logging.info(
                    "#TRAIN#:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], Loss1: {:.4f} Loss2: {:0.4f}".format(
        ...
        if (epoch) % 5 == 0:
            torch.save(
                model.state_dict(), save_path + "BBSNet_epoch_{}.pth".format(epoch)
            )
        ...
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(
            model.state_dict(), save_path + "BBSNet_epoch_{}.pth".format(epoch + 1)
        )
